...
namespace Snippet3_2
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            // Wait for all the tasks to finish with a 3 second timeout
            if (!Task.WaitAll(new Task[] { t1, t2 }, 3000))
            {
                Console.WriteLine(
                    "GenerateAESKeys and GenerateMD5Hashes are taking more than 3 seconds to complete.");
                Console.WriteLine(t1.Status.ToString());
                Console.WriteLine(t2.Status.ToString());
            }

            // Wait for the user to press a key
            // t1 and t2 could still be running at this point
            Console.ReadLine();
        }
    }
}