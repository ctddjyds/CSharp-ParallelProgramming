...
namespace Snippet5_17
{
    class Program
    {
        ...
        private static void ProduceSentences()
        {
            ...
            try
            {
                Console.WriteLine("ProduceSentences is going to run {0} times",
                    NUM_SENTENCES);
                var spinW = new SpinWait();
                // Spin 150 times
                for (int w = 0; w < 150; w++)
                {
                    spinW.SpinOnce();
        ...
        private static void CapitalizeWordsInSentences()
        {
            ...
            var spinW = new SpinWait();
            // The number of times the SpinOnce method yields
            int contextSwitches = 0;
            var sw = Stopwatch.StartNew();
            // This example uses this spinning for educational purposes
            // It isn’t a best practice
            while (!_producingSentences)
            {
                if (spinW.NextSpinWillYield)
                {
                    // The SpinWait instance
                    // is ready to initiate a context-switch
                    contextSwitches++;
                }
                spinW.SpinOnce();
            ...
            Console.WriteLine(
                "CapitalizeWordsInSentences has called SpinOnce {0} times.",
                spinW.Count);
            Console.WriteLine(
                "It yielded the time slice {0} times.", contextSwitches);
        ...
        static void Main(string[] args)
        {
            ...
            // to explain other alternatives and their problems
