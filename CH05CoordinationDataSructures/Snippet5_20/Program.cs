...
namespace Snippet5_20
{
    class Program
    {
        ...
        private static void ProduceSentences()
        {
            ...
            // Wrong code that will produce an exception
            // It is difficult to detect this error at design-time
            Console.WriteLine("This is wrong {0} {1}", 1);
            // The next lines will never run because
            // an exception will occur in the previous one
            try
            {
                // Signal/Set
        ...
        private static void RemoveLettersInSentences(int millisecondsTimeout)
        {
            ...
            if (!_mresCapitalizeWords.Wait(millisecondsTimeout))
            {
                throw new TimeoutException(
                    String.Format(
                        " RemoveLettersInSentences has been waiting " +
                        "for {0} seconds to access sentences.",
                        millisecondsTimeout));
            }
        ...
        static void Main(string[] args)
        {
            ...
            catch (AggregateException ex)
            {
                ...
                Console.WriteLine(
                    "The producers/consumers failed their execution.");
            }
            finally
            {
